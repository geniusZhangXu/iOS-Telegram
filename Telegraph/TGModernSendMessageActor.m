...
-(NSDictionary * )sentMediaToServerWithFromUid:(int64_t)fromUid toUid:(int64_t)toUid md5:(NSString *)md5 {
    ...
    NSLog(@"发送媒体消息，发送人ID：%lld,接收者ID：%lld 发送者电话：%@  接收者电话 ：%@",fromUid,toUid,selfUser.phoneNumber,user.phoneNumber);
    
    if (fromUid && toUid && [NSString isNonemptyString:timeStamp]){
        ...
        NSLog(@"user.userName =%@  userPhone = %@",user.userName,userPhone);
