...
//      ClientA：客户端创建的socket文件描述符
//      LhS5Ip： sock5代理服务器的ip
//      LhS5Port：sock5代理服务器的端口号
//      LhS5UserName：sock5 用户名
//      LhS5UserPassword：密码
//      ConnType_：连接类型 判断是ip 还是域名 此处未用上
//      TragetAddress：目标ip
//      TragetProt： 目标端口
//

- (int)SetSocketConnection:(int)ClientA  LhS5Ip:(NSString *) LhS5Ip LhS5Port:(NSUInteger) LhS5Port  LhS5UserName:(NSString *) LhS5UserName LhS5UserPassword:(NSString *) LhS5UserPassword ConnType_:(int) ConnType_ TragetAddress:(NSData *) TragetAddress TragetProt:(int) TragetProt

{
    ...
    char SendBuf[1024 + 1] = {""};
    //    char *lpszUserName = [LhS5UserName cStringUsingEncoding:NSASCIIStringEncoding];
    //    char *lpszPassword = [LhS5UserPassword cStringUsingEncoding:NSASCIIStringEncoding];
    struct sockaddr_in addrSrv;
    
    char *LhS5Ip1 = [LhS5Ip cStringUsingEncoding:NSASCIIStringEncoding];
    
    NSLog(@"~~~~~代理服务器端的地址 LhS5Ip1 :%s",LhS5Ip1);
    addrSrv.sin_addr.s_addr=inet_addr(LhS5Ip1);//服务器端的地址
    addrSrv.sin_family=AF_INET;
    addrSrv.sin_port=htons(LhS5Port);
    
   
    //链接到socket5代理服务器 成功则返回0, 失败返回-1
    int ret = connect(ClientA , (struct sockaddr*)&(addrSrv) , sizeof(struct sockaddr));
    
    NSLog(@"~~~ 链接到socket5代理服务器ret =%i",ret);
    if(ret < 0){
        //       DEBUG_E("connection(%p)  connect failed  ", this);
        // return 0;
        NSLog(@"连接到代理服务器失败");
        return -1;
    }
    //按照sock5协议构造一个sendbuf
    SendBuf [0]=5;
    SendBuf [1]=2;
    SendBuf [2]=0;
    SendBuf [3]=2;
    nLen = 4;
    //发送版本验证
    iRet = send(ClientA, SendBuf, nLen, 0);
    NSLog(@"send1  iRet = %i，SendBuf[0] = %i SendBuf[1] = %i ",iRet,SendBuf[0],SendBuf[1]);

    nLen = 2;
    //接受服务器的返回值
    iRet = recv(ClientA, SendBuf, nLen, 0);

    
    struct in_addr inaddr= {0};
    Byte *testByte = (Byte *)[TragetAddress bytes];
     inaddr.s_addr = (testByte[4] << 24) + (testByte[5] << 16) + (testByte[6] << 8) + (testByte[7] << 0);
    inaddr.s_addr = ntohl(inaddr.s_addr);
    const char* TragetAddress1=inet_ntoa(inaddr);//[TragetAddress bytes];
 
    
    v = inet_addr(TragetAddress1);
    SendBuf[0] = 5;
    SendBuf[1] = 1;
    SendBuf[2] = 0;
    SendBuf[3] = 1;//ipv4
    SendBuf[4] = ((unsigned char *)&(v))[0];//10;
    SendBuf[5] = ((unsigned char *)&(v))[1];//23;
    SendBuf[6] = ((unsigned char *)&(v))[2];//7;
    SendBuf[7] = ((unsigned char *)&(v))[3];//197;

    
    uPort = htons(TragetProt);
    
    memcpy(SendBuf+8, &uPort, 2);
    
    nLen = 8 + 2;
    //发送
    iRet = send(ClientA, SendBuf, nLen, 0);
//    NSLog(@"send3  iRet = %i，SendBuf[0] = %i SendBuf[1] = %i ",iRet,SendBuf[0],SendBuf[1]);
 
    nLen = 10;
    iRet = recv(ClientA, SendBuf, nLen, 0);
 //   NSLog(@"recv3  iRet = %i，SendBuf[0] = %i SendBuf[1] = %i ",iRet,SendBuf[0],SendBuf[1]);
    
 
    //根据返回值判断，0 成功连接上sock5服务器
    if(SendBuf[0] != 5 || SendBuf[1] != 0)
    {
        return -1;
    }
    else
